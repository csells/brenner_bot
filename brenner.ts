...
import { runRobot, runRobotStep, runRobotStress } from "./robot-mode";
...
function formatDoctorHuman(summary: {
  ...
  const order = ["ntm", "cass", "cm", "agentMail", "claude", "codex", "agy", "robotPipeline"];
  for (const key of order) {
    const check = summary.checks[key];
    if (!check) continue;
    const label = key === "agentMail" ? "agent_mail" : key === "robotPipeline" ? "robot_pipeline" : key;
...
Commands:
  version
  doctor [--json] [--manifest <path>] [--agent-mail] [--skip-ntm] [--skip-cass] [--skip-cm] [--skip-agents] [--robot]

    --robot: Zero-cost smoke test of the robot-mode pipeline (session robot-step)
    using fake agent binaries — verifies dispatch, prompt build, spawn, delta
    parse, merge, lint, and persist all still work, without a live LLM call.
    Does not verify your real configured claude/codex/agy binaries — see the
    agent CLI presence checks (claude/codex/agy) for that.
  upgrade [--version <ver>]
  ...
  session robot-step --session-dir <path> --question <s> --round <n>
               ...
               [--sequential] [--acknowledge-agent-errors]

    Run one round of a robot mode session. Outputs JSON to stdout with round
    results (adds, kills, converged, etc) and ok: false if any agent errored
    that round. Between rounds, write operator notes to
    operator_notes_round_N.md or structured beliefs to
    ...
    --acknowledge-agent-errors: required to run round N+1 if round N had an
    agent error (refuses with exit 1 otherwise, naming the failed agent).
    Rerun round N instead to retry it, or pass this flag to proceed without it.
...
// ============================================================================
// Robot pipeline smoke check (doctor --robot)
//
// Formalizes the zero-cost verification trick used throughout this branch's
// development: fake agent binaries stand in for claude/codex/agy so the full
// round pipeline — dispatch, prompt build, spawn, delta parse, merge, lint,
// persist, JSON shape — is exercised for real, at zero API cost. This does
// NOT verify the real configured agent binaries work; it verifies the
// pipeline itself hasn't regressed. See the "doctor" command's agent CLI
// presence checks for real-binary verification.
// ============================================================================

function writeFakeCodexBin(dir: string): string {
  // RedForest reads its output from a file path (--output-last-message
  // <path>), not stdout, so plain `echo` can't stand in for it like it can
  // for BlueLake/GreenMountain. Write a tiny script that does.
  const scriptPath = join(dir, "fake-codex.sh");
  writeFileSync(scriptPath, `#!/bin/sh\necho "fake codex output for: $5" > "$4"\n`, { mode: 0o755 });
  return scriptPath;
}

async function runRobotPipelineSmokeCheck(): Promise<DoctorCheck> {
  const scratchDir = join(tmpdir(), `brenner-doctor-robot-${randomUUID()}`);
  mkdirSync(scratchDir, { recursive: true });
  try {
    const sessionDir = join(scratchDir, "session");
    mkdirSync(sessionDir, { recursive: true });
    writeFileSync(
      join(sessionDir, "context.md"),
      "## Research Question\ndoctor --robot smoke check\n\n## Background\nN/A\n\n## Evaluation Criteria\nN/A\n",
    );
    writeFileSync(join(sessionDir, "excerpt.md"), "");
    const fakeCodexBin = writeFakeCodexBin(scratchDir);

    const args = [
      "run", import.meta.path,
      "session", "robot-step",
      "--session-dir", sessionDir,
      "--question", "doctor --robot smoke check",
      "--round", "1",
      "--claude-bin", "/bin/echo",
      "--codex-bin", fakeCodexBin,
      "--agy-bin", "/bin/echo",
      "--json",
    ];

    const result = await new Promise<{ exitCode: number | null; stdout: string; stderr: string }>((res) => {
      ...
      const timeout = setTimeout(() => {
        child.kill("SIGKILL");
        res({ exitCode: null, stdout, stderr: `${stderr}\n[!] smoke check timed out after 20s` });
      }, 20_000);
      child.on("close", (code) => {
      ...
      child.on("error", (err: Error) => {
        clearTimeout(timeout);
        res({ exitCode: null, stdout, stderr: err.message });
      });
    });
    ...
    if (result.exitCode !== 0) {
      return {
        status: "error",
        path: null,
        verifyCommand: "session robot-step (fake agents)",
        exitCode: result.exitCode,
        notes: "Robot-mode pipeline did not complete a round cleanly.",
        stderr: result.stderr.slice(0, 2000),
      };
    }

    let parsed: { round?: number; artifactFile?: string; stateFile?: string; agents?: unknown };
    try {
      ...
      return {
        ...
        verifyCommand: "session robot-step (fake agents)",
        exitCode: result.exitCode,
        notes: `Round completed but JSON output did not parse: ${msg}`,
      };
    }

    if (!parsed.artifactFile || !existsSync(parsed.artifactFile) || !parsed.stateFile || !existsSync(parsed.stateFile)) {
      return {
        status: "error",
        path: null,
        verifyCommand: "session robot-step (fake agents)",
        exitCode: result.exitCode,
        notes: "Round reported success but did not persist artifact.md / session_state.json.",
      };
    }
    ...
    return {
      ...
      verifyCommand: "session robot-step (fake agents)",
      exitCode: result.exitCode,
      notes: `Round ${parsed.round ?? "?"} pipeline ran end to end (dispatch, prompt build, spawn, delta parse, merge, lint, persist).`,
    };
  } finally {
...
async function main(): Promise<void> {
  ...
  if (top === "doctor") {
    ...
    const checkRobot = asBoolFlag(flags, "robot");
    ...
    if (checkRobot) {
      checks.robotPipeline = await runRobotPipelineSmokeCheck();
    }

    // Only ntm, cass, cm are potentially required; agent CLIs are optional.
    // robotPipeline is opt-in (only present when --robot was passed) but
    // required once requested — if you ask doctor to check it, a failure
    // should fail the doctor run, not sit unnoticed in the checks object.
    const requiredToolKeys = ["ntm", "cass", "cm", "robotPipeline"];
...
`;
  ...
  // ============================================================================
  // Robot mode: session robot / robot-step / robot-stress
  //
  // Implementations live in robot-mode.ts — that file plus its exports is the
  // whole robot-mode surface, nothing related hides elsewhere in this file.
  // ============================================================================
  if (normalizedTop === "session" && sub === "robot-step") {
    await runRobotStep(flags);
    return;
  }

  if (normalizedTop === "session" && sub === "robot-stress") {
    await runRobotStress(flags);
    return;
  }

  if (normalizedTop === "session" && sub === "robot") {
    await runRobot(flags);
    return;
  }
